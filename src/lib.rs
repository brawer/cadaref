...
impl Matcher {
    pub fn new(points: PathBuf, symbols: PathBuf) -> Result<Self, Box<dyn Error>> {
		...
		let matcher = Matcher{points, point_distances, point_tree, symbols};
		Ok(matcher)
	}
	...
	pub fn find_matches(&self, meters_per_pixel: f64) {
		for i in 0..self.symbols.len() {
			for j in i+1..self.symbols.len() {
				...
				println!("i={:?} j={:?} dist_mm={dist_mm}", self.symbols[i], self.symbols[j]);
			}
		}
	}
}
...
fn sort_points(points: &mut [Point]) {
   if points.len() < 2 {
	...
	let x0 = points.iter().min_by(|a, b| {
		a.x.partial_cmp(&b.x).unwrap()
	}).unwrap().x;
	let y0 = points.iter().min_by(|a, b| {
...
fn sort_symbols(symbols: &mut [Symbol]) {
   if symbols.len() < 2 {
	...
	let x0 = symbols.iter().min_by(|a, b| {
		a.x.partial_cmp(&b.x).unwrap()
	}).unwrap().x;
	let y0 = symbols.iter().min_by(|a, b| {
		a.y.partial_cmp(&b.y).unwrap()
	}).unwrap().y;
	symbols.sort_by(|a, b| {
	    let da = (a.x - x0) * (a.x - x0) + (a.y - y0) * (a.y - y0);
...
mod tests {
    ...
    fn can_sort_points() {
	    let mut points = vec![
		    Point{id:"A".to_string(), x:4., y:2.},
		    Point{id:"B".to_string(), x:77., y:99.0},
		    Point{id:"C".to_string(), x:11., y:33.0},
		];
		sort_points(&mut points);
    ...
    fn can_make_gcp() {
		let gcp = make_gcp(
			&Symbol{x: 1723.25, y: 3282.0},
			&Point{id: "HGF4845".to_string(), x: 2679878.122, y:1251033.022},
		);
    ...
    fn can_build_point_distances() {
	   let points = vec![
 	   	   Point{id: "HGF4845".to_string(), x: 2679878.122, y: 1251033.022},
		   Point{id: "HGC4303".to_string(), x: 2678592.752, y: 1251921.679},
		   Point{id: "HGB4303".to_string(), x: 2678599.342, y: 1251903.290},
	   ];
